export default function Bouquet() {
  const flowers = [
    ...
    { 
      ...
      isWide: true,
      iconSet: [
        { i: 'yard', c: 'text-secondary', s: '70px' },
        { i: 'local_florist', c: 'text-tertiary', s: '80px' },
        { i: 'spa', c: 'text-primary', s: '70px' }
      ],
      colorRef: 'primary', 
  ...
  return (
    <div className="flex-1 px-4 md:px-gutter py-8 md:py-12 max-w-container-max mx-auto pb-24 md:pb-32 w-full">
      <section className="mb-10 md:mb-12 text-center max-w-2xl mx-auto pt-4 md:pt-8">
        <h1 className="font-display-lg text-[48px] md:text-[64px] text-on-surface mb-3 md:mb-4 drop-shadow-sm leading-none">Pick a Flower</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 flower-pattern px-2 md:px-4 py-8 rounded-[1.5rem] md:rounded-3xl">
        {flowers.map((f, i) => (
          <div key={i} className={`flip-card h-80 rounded-3xl cursor-pointer ${f.isWide ? 'lg:col-span-2' : ''}`}>
            <div className="flip-card-inner w-full h-full rounded-[1.5rem] relative">
              <div className={`flip-card-front absolute w-full h-full bg-surface-container-low rounded-[1.5rem] shadow-[0_20px_40px_rgba(0,0,0,0.04)] flex flex-col items-center justify-center p-6 border transition-colors bg-gradient-to-br from-surface to-surface-container-low ${colorStyles[f.colorRef].split(' ')[0]} ${colorStyles[f.colorRef].split(' ')[1]}`}>
                {f.isWide && f.iconSet ? (
                  <div className="flex gap-2 mb-4 drop-shadow-md items-end">
                  ...
                  <span className={`material-symbols-outlined mb-4 drop-shadow-md ${colorStyles[f.colorRef].split(' ')[2]}`} style={{ fontSize: '100px', fontVariationSettings: "'FILL' 1" }}>{f.icon}</span>
                )}
                <h3 className="font-display-lg text-2xl text-on-surface tracking-tight font-semibold mt-2">{f.name}</h3>
              </div>

              <div className={`flip-card-back absolute w-full h-full rounded-[1.5rem] shadow-xl flex flex-col items-center justify-center p-8 text-center border-2 border-transparent ${colorStyles[f.colorRef].split(' ').slice(3).join(' ')}`}>
                <span className="material-symbols-outlined mb-4 text-4xl opacity-80" style={{ fontVariationSettings: "'FILL' 1" }}>favorite</span>
                <p className="font-body-lg font-semibold">{f.msg}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="mt-margin flex justify-center pb-20 px-4">
        <button className="bg-primary text-white rounded-full px-8 py-4 font-sans text-[12px] font-semibold uppercase tracking-[0.1em] hover:bg-black transition-all duration-300 shadow-[0_10px_20px_rgba(0,0,0,0.05)] flex items-center justify-center gap-2 w-full sm:w-auto mt-4 md:mt-0">
